...
func GetAnswerFor(intent string) string {
	...
	faq, err := GetAnswerByIntent(intent)
	if err != nil {
		return ""
	}
...
func GetAnswerByIntent(intent string) (FAQ, error) {
	...
	query := "SELECT * FROM intent_answer WHERE intent=?;"
	stmt, err := DB.Prepare(query)
	if err != nil {
		log.Error("failed to prepare GetAnswerByIntent query: ", err)
		return FAQ{}, err
	}
	defer stmt.Close()

	rows, err := stmt.Query(intent)
	if err != nil {
		log.Error("failed to execute GetAnswerByIntent query: ", err)
		return FAQ{}, err
	}
	defer rows.Close()
